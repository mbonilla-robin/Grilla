...
export function BrandConfigEditor({
  ...
  async function handleSave() {
    ...
    const strategy = await updateBrandStrategy(orgId, {
      tone_of_voice: tone,
      objective,
    });
    if (strategy.error) {
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <section className="home-card p-5 space-y-4">
        ...
        {pillars.length === 0 ? (
          <p className="text-sm text-muted py-4 text-center">
            Sin pilares configurados. Agrega los temas que guían el contenido de esta marca.
          </p>
        ) : (
      ...
      {canEdit ? (
        ...
        <p className="text-xs text-muted">
          Solo admins y creadores pueden editar la configuración de marca.
        </p>
      )}
    </div>
  );
